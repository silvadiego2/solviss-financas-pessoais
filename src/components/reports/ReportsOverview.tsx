    ...
    <div className="space-y-4">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthly} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tickFormatter={(v) => `R$${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} width={52} />
                  <Tooltip content={<TooltipBar />} />
                  <Legend formatter={(v) => v === 'receitas' ? 'Receitas' : 'Despesas'} wrapperStyle={{ fontSize: 12 }} />
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-10">Nenhuma despesa encontrada este mês</p>
          ) : (
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <div className="w-full sm:w-1/2 h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={byCategory} cx="50%" cy="50%" innerRadius={48} outerRadius={80} paddingAngle={2} strokeWidth={0} dataKey="value">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {isLoading ? (
            Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-5 w-full rounded" />)
          ) : byCategory.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">Nenhuma despesa encontrada para este período</p>
          ) : (
            byCategory.map((cat, i) => (
