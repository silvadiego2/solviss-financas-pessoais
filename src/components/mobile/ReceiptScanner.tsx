...
import { Camera, Upload, Loader2, FileImage, DollarSign, Trash2 } from 'lucide-react';
...
export const ReceiptScanner: React.FC<ReceiptScannerProps> = ({ onBack, onTransactionCreated }) => {
  ...
  const extractReceiptData = (text: string): ScannedData => {
    ...
    if (amounts && amounts.length > 0) {
      ...
      if (numericAmounts.length > 0) amount = Math.max(...numericAmounts);
    }
  ...
  return (
    <div className="space-y-6">
      <BackHeader
        ...
        icon={<Camera size={22} />}
        subtitle="Capture recibos e crie transações automaticamente"
        onBack={onBack}
      />
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex justify-center">
              <img src={capturedImage} alt="Recibo capturado" className="max-w-full max-h-96 rounded-lg border shadow-md" />
            </div>
            {isProcessing && (
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-2">
                ...
                <Input id="amount" type="number" step="0.01" value={editedData.amount || ''}
                  onChange={(e) => setEditedData(prev => prev ? {...prev, amount: parseFloat(e.target.value) || 0} : null)}
                  placeholder="0,00" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="description">Descrição</Label>
                <Input id="description" value={editedData.description || ''}
                  onChange={(e) => setEditedData(prev => prev ? {...prev, description: e.target.value} : null)}
                  placeholder="Descrição da transação" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="date">Data</Label>
                <Input id="date" type="date" value={editedData.date || ''}
                  onChange={(e) => setEditedData(prev => prev ? {...prev, date: e.target.value} : null)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="account">Conta *</Label>
                <Select value={editedData.account_id || ''} onValueChange={(value) => setEditedData(prev => prev ? {...prev, account_id: value} : null)}>
              ...
              <div className="space-y-2">
                ...
                <Select value={editedData.category_id || ''} onValueChange={(value) => setEditedData(prev => prev ? {...prev, category_id: value} : null)}>
              ...
              <div className="space-y-2">
                ...
                <Input id="merchant" value={editedData.merchant || ''}
                  onChange={(e) => setEditedData(prev => prev ? {...prev, merchant: e.target.value} : null)}
                  placeholder="Nome do estabelecimento" />
              </div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={resetScanner} className="flex-1" disabled={isCreating}>
                  <Trash2 size={16} className="mr-2" />
                  Descartar
                </Button>
                <Button onClick={handleCreateTransaction} className="flex-1" disabled={isCreating}>
                  {isCreating ? <Loader2 size={16} className="mr-2 animate-spin" /> : <DollarSign size={16} className="mr-2" />}
